...
if not LPH_OBFUSCATED then
    ...
    SWG_DiscordUser = "OCTO"
    SWG_DiscordID = 1337
    SWG_Private = true
    SWG_Dev = false
    SWG_Version = "paid"
    SWG_Title = 'OCTO.HOOK / some10 (.gg/priv8)'
    SWG_ShortName = 'CUSTOM'
    SWG_FullName = 'ts'
    SWG_FFA = false
end;
...
local function getswimhubasset(file)
    ...
    local success, returns = pcall(getcustomasset, "octo,hook/new/files/"..file)
    if success then return returns else return print(returns) end
end
do
    if not isfolder("octo,hook") then makefolder("octo,hook") end
    if not isfolder("octo,hook/new") then makefolder("octo,hook/new") end
    if not isfolder("octo,hook/new/files") then makefolder("octo,hook/new/files") end
    local function getfiles(force, list)
        for _, file in list do
            if (force or not force and not isswimhubfile(file)) then
                writefile("octo,hook/new/files/"..file, getfile(file))
            end
        end
    end
    ...
    if localassets then
        ...
        if localassets.version ~= assets.version then
            writefile("octo,hook/new/files/assets.json", gotassets)
...
ui.tabs = {
    combat = ui.window:AddTab('PVP'),
    visuals = ui.window:AddTab('WH'),
    misc = ui.window:AddTab('misc'),
    config = ui.window:AddTab('config'),
    OCTO = ui.window:AddTab('OCTO'),
}
ui.box = {
    ...
    Fov120= ui.tabs.OCTO:AddLeftTabbox(),
    Rage = ui.tabs.OCTO:AddRightTabbox(),
    jumpshoot = ui.tabs.OCTO:AddLeftTabbox(),
    hum = ui.tabs.OCTO:AddLeftTabbox(),
}
...
do
    ...
    hbb:AddSlider('hitbox_head_size_x', { Text = 'size x', Default = 10, Min = 1, Max = 40, Rounding = 1, Compact = false }):OnChanged(function(State)
        hitboxheadsizex = State
        hbsize = _Vector3new(hitboxheadsizex, hitboxheadsizey, hitboxheadsizex)
    end)
    hbb:AddSlider('hitbox_head_size_y', { Text = 'size y', Default = 10, Min = 1, Max = 40, Rounding = 1, Compact = false }):OnChanged(function(State)
...
do
    ...
    "OCTO.HOOK canary | Jul 28 2025 | fps: 240 ";
    ...
    if canuseimages then rawget(gradr, "_frame").Image = getcustomasset("swim/new/files/grad90r.png"); end
    if canuseimages then rawget(gradl, "_frame").Image = getcustomasset("swim/new/files/grad90l.png"); end
    ...
    cheat.utility.new_renderstepped(LPH_JIT_MAX(function(delta)
        ...
        watertext = ("OCTO %s | %s | %s | %s fps"):format(SWG_Version, SWG_ShortName, os.date("%b %d %Y"), tostring(finalfps));
...
do
    ...
    mvb:AddSlider('speedhack_speed',{ Text = 'speed', Default = 55, Min = 55, Max = 80, Rounding = 0, Suffix = "sps", Compact = false }):OnChanged(function(State)
        speed = State
    end)
...
do
    ...
    mvb:AddSlider('freecam_speed',{ Text = 'speed', Default = 10, Min = 1, Max = 600, Rounding = 0, Suffix = "sps", Compact = false }):OnChanged(function(State)
        speed = State
    end)
...
-- Создаем вкладку Anti-Aim
local antiAimTab = ui.box.misc:AddTab("Anti Aim")

-- Настройки Anti-Aim
local antiAim = {
    enabled = false,
    type = "Jitter", -- "Jitter", "Spin", "Static", "Random"
    speed = 5,
    angle = 60,
    desync = false,
    desyncAmount = 60,
    lastUpdate = 0,
    invert = false,
    pitch = "None" -- "None", "Down", "Up", "Random"
}

-- Сохраняем оригинальное положение головы
local originalPrismCFrame = workspace.Const.Ignore.LocalCharacter.Top.Prism1.CFrame

-- Элементы управления Anti-Aim
antiAimTab:AddToggle('antiAim_enabled', {
    Text = 'Enable Anti-Aim',
    Default = false,
    Callback = function(state)
        antiAim.enabled = state
        if not state then
            -- Возвращаем голову в исходное положение при отключении
            workspace.Const.Ignore.LocalCharacter.Top.Prism1.CFrame = originalPrismCFrame
        end
    end
}):AddKeyPicker('antiAim_keybind', {
    Default = 'None',
    SyncToggleState = true,
    Mode = 'Toggle',
    Text = 'Anti-Aim Toggle',
    NoUI = false
})

antiAimTab:AddDropdown('antiAim_type', {
    Values = {"Jitter", "Spin", "Static", "Random"},
    Default = 1,
    Multi = false,
    Text = 'Anti-Aim Type',
    Callback = function(value)
...
antiAimTab:AddSlider('antiAim_speed', {
    Text = 'Speed',
    Default = 5,
    Min = 1,
    Max = 20,
...
antiAimTab:AddSlider('antiAim_angle', {
    Text = 'Angle',
    Default = 60,
    Min = 15,
    Max = 180,
...
antiAimTab:AddToggle('antiAim_invert', {
    Text = 'Invert Direction',
    Default = false,
    Callback = function(state)
        antiAim.invert = state
    end
})

antiAimTab:AddDropdown('antiAim_pitch', {
    Values = {"None", "Down", "Up", "Random"},
    Default = 1,
    Multi = false,
    Text = 'Pitch Control',
    Callback = function(value)
...
antiAimTab:AddToggle('antiAim_desync', {
    Text = 'Enable Desync',
    Default = false,
    Callback = function(state)
        antiAim.desync = state
    end
})

antiAimTab:AddSlider('antiAim_desyncAmount', {
    Text = 'Desync Amount',
    Default = 60,
    Min = 10,
    Max = 180,
...
local function applyAntiAim()
    ...
    local angleRad = math.rad(antiAim.angle)
    local direction = antiAim.invert and -1 or 1
    
    -- Вычисляем поворот в зависимости от типа Anti-Aim
    local yaw = 0
    if antiAim.type == "Jitter" then
        yaw = math.sin(currentTime * antiAim.speed) * angleRad * direction
    elseif antiAim.type == "Spin" then
        yaw = (currentTime * antiAim.speed * direction) % (2 * math.pi)
    elseif antiAim.type == "Static" then
        yaw = angleRad * direction
    elseif antiAim.type == "Random" then
        if currentTime - antiAim.lastUpdate > 0.5 then
            antiAim.lastUpdate = currentTime
            yaw = math.rad(math.random(-antiAim.angle, antiAim.angle)) * direction
        end
    end
    
    -- Добавляем десинхронизацию если включена
    if antiAim.desync then
        local desyncOffset = math.sin(currentTime * 5) * math.rad(antiAim.desyncAmount)
        yaw = yaw + desyncOffset
    end
    
    -- Управление pitch (наклоном головы)
    local pitch = 0
    if antiAim.pitch == "Down" then
        pitch = math.rad(-90)
    elseif antiAim.pitch == "Up" then
        pitch = math.rad(90)
    elseif antiAim.pitch == "Random" then
        if currentTime - antiAim.lastUpdate > 0.5 then
            pitch = math.rad(math.random(-90, 90))
        end
    end
    
    -- Применяем поворот к призме (голове)
    prism.CFrame = originalPrismCFrame * CFrame.Angles(pitch, yaw, 0)
end

-- Запускаем Anti-Aim каждый кадр
cheat.utility.new_heartbeat(applyAntiAim)

-- Информационные метки
antiAimTab:AddDivider()
antiAimTab:AddLabel("Anti-Aim делает вашу голову")
antiAimTab:AddLabel("непредсказуемой для врагов")
antiAimTab:AddLabel("Jitter - Дрожание")
antiAimTab:AddLabel("Spin - Вращение")
antiAimTab:AddLabel("Static - Фиксированный угол")
antiAimTab:AddLabel("Random - Случайные углы")



-- Добавляем FOV чейнгер во вкладку OCTO
local fovTab = ui.box.Fov120:AddTab("FOV Changer")
...
-- Элементы управления FOV
fovTab:AddToggle('fov_enabled', {
    Text = 'Enable FOV Changer',
    Default = false,
    Callback = function(state)
        fovSettings.enabled = state
        if not state then
            -- Возвращаем оригинальный FOV при отключении
            workspace.CurrentCamera.FieldOfView = fovSettings.oldFov
        end
    end
})

fovTab:AddSlider('fov_value', {
    Text = 'FOV Value',
    Default = 90,
    Min = 70,
    Max = 120,
...
local function updateFOV()
    if fovSettings.enabled then
        workspace.CurrentCamera.FieldOfView = fovSettings.fovValue
    end
end

-- Запускаем обновление FOV каждый кадр
cheat.utility.new_renderstepped(updateFOV)
...
local success, fogTab = pcall(function()
    return ui.box.Fov120:AddTab("Fog Settings")
end)

if not success then
    fogTab = nil
    warn("[Fog] UI blocked, running in silent mode")
end
...
-- Если вкладка создана, добавляем элементы
if fogTab then
    fogTab:AddToggle('fog_enabled', {
        Text = 'Enable Fog',
        Default = false,
        Callback = function(state)
            fogSettings.enabled = state
        end
    }):AddColorPicker('fog_color', {
        Default = fogSettings.color,
        Title = 'Fog Color',
        Transparency = 0,
        Callback = function(value)
            fogSettings.color = value
        end
    })

    fogTab:AddSlider('fog_density', {
        Text = 'Fog Density',
        Default = 0.1,
        Min = 0,
        Max = 1,
        ...
        Compact = false,
        Callback = function(value)
            fogSettings.density = value
        end
    })

    fogTab:AddSlider('fog_start', {
        ...
        Compact = false,
        Callback = function(value)
            fogSettings.startDistance = value
        end
    })

    fogTab:AddSlider('fog_end', {
        ...
        Compact = false,
        Callback = function(value)
            fogSettings.endDistance = value
        end
    })
end

-- Получаем Lighting безопасным способом
local Lighting = game:GetService("Lighting")

-- Плавное применение тумана с рандомными задержками
task.spawn(function()
    while true do
        if fogSettings.enabled then
            -- Добавляем небольшой "шум" к значениям, чтобы скрыть паттерн
            local noise = (math.random() - 0.5) * 2  -- от -1 до 1
            local adjustedStart = fogSettings.startDistance + noise * 5
            local adjustedEnd = fogSettings.endDistance + noise * 5

            -- Пытаемся применить настройки с обработкой ошибок
            pcall(function()
                Lighting.FogColor = fogSettings.color
                Lighting.FogStart = adjustedStart
                Lighting.FogEnd = adjustedEnd
        ...
        -- Рандомная задержка (от 0.001 до 0.01 секунды)
        task.wait(math.random() * 0.009 + 0.001)
    end
end)
...
-- Добавляем вкладку Magic Bullet в Rage
local rageMagicTab = ui.box.Rage:AddTab("Magic Bullet")

-- Настройки Magic Bullet для Trident Survival
local magicSettings = {
    enabled = false,
    onlyInFOV = true,
    autoFire = false,
...
-- Элементы управления Magic Bullet
rageMagicTab:AddToggle('magic_enabled', {
    Text = 'Enable Magic Bullet',
    Default = false,
    Callback = function(state)
        magicSettings.enabled = state
    end
}):AddKeyPicker('magic_keybind', {
    Default = 'None',
    SyncToggleState = true,
    Mode = 'Toggle',
    Text = 'Magic Bullet Toggle',
    NoUI = false
})

rageMagicTab:AddToggle('magic_onlyfov', {
    Text = 'Only in FOV',
    Default = true,
    Tooltip = 'Активировать только в пределах FOV круга',
    Callback = function(state)
        magicSettings.onlyInFOV = state
    end
})

rageMagicTab:AddToggle('magic_auto', {
    Text = 'Auto Fire',
    Default = false,
    Callback = function(state)
        magicSettings.autoFire = state
    end
})

rageMagicTab:AddSlider('magic_hitchance', {
    Text = 'Hit Chance',
    Default = 95,
    Min = 50,
    Max = 100,
    ...
    Compact = false,
    Callback = function(value)
        magicSettings.hitChance = value
    end
})

rageMagicTab:AddSlider('magic_distance', {
    Text = 'Max Distance',
    Default = 500,
    Min = 50,
    Max = 2000,
    ...
    Compact = false,
    Callback = function(value)
        magicSettings.maxDistance = value
    end
})

-- Добавляем вкладку Always Hit в Rage
local alwaysHitTab = ui.box.Rage:AddTab("Always Hit")

-- Настройки Always Hit для Trident Survival
local alwaysHitSettings = {
    enabled = false,
    onlyInFOV = true,
    hitboxExtender = true,
    hitboxSize = 1.5,
...
-- Элементы управления Always Hit
alwaysHitTab:AddToggle('alwayshit_enabled', {
    Text = 'Enable Always Hit',
    Default = false,
    Callback = function(state)
        alwaysHitSettings.enabled = state
        if not state then
...
alwaysHitTab:AddToggle('alwayshit_onlyfov', {
    Text = 'Only in FOV',
    Default = true,
    Tooltip = 'Активировать только в пределах FOV круга',
    Callback = function(state)
        alwaysHitSettings.onlyInFOV = state
    end
})

alwaysHitTab:AddSlider('alwayshit_size', {
    Text = 'Hitbox Size',
    Default = 1.5,
    Min = 1,
    Max = 2.5,
    ...
    Compact = false,
    Callback = function(value)
        alwaysHitSettings.hitboxSize = value
    end
})

-- Функция проверки нахождения в FOV
local function isInFOV(position)
    local screenPos, onScreen = _WorldToViewportPoint(Camera, position)
    if not onScreen then return false end
    
    local center = _Vector2new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2)
    local distance = (_Vector2new(screenPos.X, screenPos.Y) - center).Magnitude
    
    return distance <= magicSettings.fovSize
end

-- Рабочий Magic Bullet для Trident Survival
local function workingMagicBullet()
    if not magicSettings.enabled then return end
    if tick() - magicSettings.lastShot < magicSettings.shotDelay then return end
    
    -- Получаем цель с учетом FOV
    local targetPart, targetPos = get_closest_target(magicSettings.onlyInFOV, magicSettings.fovSize)
    if not targetPart then return end
    if magicSettings.onlyInFOV and not isInFOV(targetPart.Position) then return end
    
    -- Проверяем шанс попадания
    if math.random(1, 100) > magicSettings.hitChance then return end
    
    -- Добавляем небольшое случайное отклонение
    local randomOffset = Vector3.new(
        (math.random() - 0.5) * magicSettings.randomOffset,
        (math.random() - 0.5) * magicSettings.randomOffset,
        (math.random() - 0.5) * magicSettings.randomOffset
    )
    
    -- Корректный выстрел для Trident Survival
    local fireArgs = {
        [1] = "Fire", -- Тип действия
        [2] = targetPos + randomOffset, -- Позиция выстрела
        [3] = CFrame.new(Camera.CFrame.Position, targetPos), -- Направление
        [4] = tick(), -- Таймстамп
        [5] = math.random(1000, 9999) -- Уникальный ID выстрела
    }
    
    -- Отправляем выстрел с правильными аргументами
    trident.tcp:FireServer(unpack(fireArgs))
    
    magicSettings.lastShot = tick()
end

-- Рабочий Always Hit для Trident Survival
local function workingAlwaysHit()
    if not alwaysHitSettings.enabled then return end
    if tick() - alwaysHitSettings.lastUpdate < alwaysHitSettings.updateInterval then return end
    
    for _, char in pairs(validcharacters) do
        local head = char:FindFirstChild("Head")
        if head then
            -- Проверяем расстояние и FOV
            local distance = (head.Position - Camera.CFrame.Position).Magnitude
            local inFOV = not alwaysHitSettings.onlyInFOV or isInFOV(head.Position)
            
            if distance <= alwaysHitSettings.maxDistance and inFOV then
                -- Плавное изменение размера хитбокса
                local targetSize = alwaysHitSettings.hitboxSize
                local currentSize = head.Size.X
                local newSize = currentSize + (targetSize - currentSize) * 0.3
                
                head.Size = Vector3.new(newSize, newSize, newSize)
            else
                -- Плавное возвращение к нормальному размеру
                if head.Size.X > 1.01 then
                    head.Size = head.Size:Lerp(Vector3.new(1,1,1), 0.3)
                end
            end
        end
    end
    
    alwaysHitSettings.lastUpdate = tick()
end

-- Синхронизация FOV с Aimbot
cheat.utility.new_heartbeat(function()
    magicSettings.fovSize = fov_size
    alwaysHitSettings.fovSize = fov_size
end)

-- Подключаем обработчики
cheat.utility.new_renderstepped(function()
    pcall(workingMagicBullet)
end)

cheat.utility.new_heartbeat(function()
    pcall(workingAlwaysHit)
end)

-- Информация для пользователя
rageMagicTab:AddDivider()
rageMagicTab:AddLabel("Magic Bullet теперь корректно стреляет")
rageMagicTab:AddLabel("в целях внутри FOV круга")

alwaysHitTab:AddDivider()
alwaysHitTab:AddLabel("Always Hit плавно изменяет хитбоксы")
alwaysHitTab:AddLabel("только для целей в FOV круга")
...
local vehicleFlyTab = ui.box.hum:AddTab("Vehicle Fly & NoClip")
...
local function scanForVehicles()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        -- Ищем объекты с сиденьями или признаками транспорта
...
-- Включение системы
vehicleFlyTab:AddToggle('vehicle_fly', {
    Text = 'Включить полёт',
    Default = false,
    Callback = function(state)
        ...
        if state then
            scanForVehicles()
            cheat.Library:Notify("Найдено транспорта: "..#vehicleSettings.activeVehicles, 2)
        else
            -- Останавливаем все транспортные средства
...
-- NoClip для транспорта
vehicleFlyTab:AddToggle('vehicle_noclip', {
        ...
        for _, vehicle in pairs(vehicleSettings.activeVehicles) do
            for _, part in pairs(vehicle.model:GetDescendants()) do
                if part:IsA("BasePart") then
                    ...
                    if state then
                        part.AssemblyLinearVelocity = Vector3.new(0, 5, 0) -- Подъём при активации
                    end
                end
            end
        end
...
cheat.utility.new_heartbeat(function()
    ...
    -- Управление для всех транспортных средств
    for _, vehicle in pairs(vehicleSettings.activeVehicles) do
        ...
        -- Если транспорт может двигаться без водителя или есть водитель
        if hasDriver or not vehicle.seat then
            ...
            elseif _IsKeyDown(UserInputService, Enum.KeyCode.S) then
                velocity -= moveDir * vehicleSettings.speed
            end
            
            if _IsKeyDown(UserInputService, Enum.KeyCode.A) then
                velocity -= Vector3.new(-moveDir.Z, 0, moveDir.X) * vehicleSettings.speed * 0.7
            elseif _IsKeyDown(UserInputService, Enum.KeyCode.D) then
                velocity += Vector3.new(-moveDir.Z, 0, moveDir.X) * vehicleSettings.speed * 0.7
            end
            
            -- Вертикальное движение
            if _IsKeyDown(UserInputService, Enum.KeyCode.Space) then
                velocity += Vector3.new(0, vehicleSettings.verticalSpeed, 0)
            elseif _IsKeyDown(UserInputService, Enum.KeyCode.LeftControl) then
                velocity -= Vector3.new(0, vehicleSettings.verticalSpeed, 0)
            end
...
-- Настройки скорости
vehicleFlyTab:AddSlider('vehicle_speed', {
    ...
    Callback = function(val)
        vehicleSettings.speed = val
    end
})

vehicleFlyTab:AddSlider('vehicle_vspeed', {
    ...
    Suffix = " studs/s",
    Callback = function(val)
        vehicleSettings.verticalSpeed = val
    end
})

-- Инструкция
vehicleFlyTab:AddDivider()
vehicleFlyTab:AddLabel("Инструкция:")
vehicleFlyTab:AddLabel("1. Включите полёт и/или NoClip")
vehicleFlyTab:AddLabel("2. Управление:")
vehicleFlyTab:AddLabel("   - WASD: движение")
vehicleFlyTab:AddLabel("   - Space: вверх")
vehicleFlyTab:AddLabel("   - LCtrl: вниз")
vehicleFlyTab:AddLabel("3. Работает на всех транспортных средствах")
